        ...
        try:
            while aux!=1:  
                try:
                    ...
                    if len(buscarNoTex):
                        respuesta = respuesta + "(%i0) " + expresion
                        continue
                    ###print("Sending "+ expresion+"\n")
                    connection.sendall(str.encode(expresion+"\n"))
                    # Receive the data in small chunks and retransmit it
...
def procesar():
    if request.method == 'POST':
        ...
        respuestaJson={"salidas":listaRespuestas,"echoEntradas":echoSpeech,"entradaNoEcho":entradaNoEcho,"salidaSpeech":salidaSpeech}
